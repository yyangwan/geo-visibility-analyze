...
description: Translate an existing Remotion (React-based) video composition into a HyperFrames HTML composition. Use only when the user explicitly asks to port, convert, migrate, translate, or rewrite an existing Remotion composition as HyperFrames. Do not use for new HyperFrames compositions, for Remotion code shared only as reference, or when the user wants a similar result without migrating the source. If unsupported patterns such as useState, side-effectful useEffect, async calculateMetadata, third-party React UI libraries, or `@remotion/lambda` features appear, stop and recommend the runtime interop escape hatch instead of attempting a lossy translation.
---
